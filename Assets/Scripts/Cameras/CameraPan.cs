...
public class CameraPan : MonoBehaviour {
    ...
    public float movemnetThreshold = .25f; // Percentage offset where move starts (25%)
    ...
    // Get incoming position from getadjustedpos method
    ...
    void Movement()
    {
        ...
        Transform camTransform = attatchedCamera.transform;


        // Get mouse to viewport coordinates
        Vector2 mousePoint = attatchedCamera.ScreenToViewportPoint(Input.mousePosition);
        Vector2 offset = mousePoint - new Vector2(.5f, .5f);

        Vector3 input = Vector3.zero;
        if (offset.magnitude > movemnetThreshold)
